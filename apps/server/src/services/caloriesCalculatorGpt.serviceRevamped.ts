...
async function scrappingCalculator(
  ...
  try {
    ...
    console.log(
      '[scrappingCalculator] Logging into Microsoft with provided credentials'
    )
    ...
    if (!response) {
      ...
      while (!response && parseRetries < maxParseRetries) {
        ...
        console.log(
          '[scrappingCalculator] Response from reminder prompt attempt:',
          response
        )
        parseRetries++
      }
    ...
    if (!response && attempts < maxRetries - 1) {
      console.warn(
        '[scrappingCalculator] No valid response received. Retrying full process.'
      )
